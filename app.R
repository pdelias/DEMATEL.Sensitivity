# Spectral DEMATEL -- Shiny application
# File: app.R
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tags$head(
      ...
      # Under shinylive the application runs inside an iframe, and the tab title
      # belongs to shinylive's own index.html, which says "Shiny App". That page
      # is generated by the export and there is no argument to change it, so
      # setting it from here is what survives a rebuild -- a post-processing
      # step would have to be remembered every time. Same origin, so the parent
      # is reachable; wrapped anyway, because a title is not worth an exception.
      tags$script(HTML(
        "try { document.title = 'Spectral DEMATEL';",
    ...
    content = function(file) {
      if (!is.null(values$sensitivity_results)) {
        ...
        report_content <- paste(
          "SPECTRAL DEMATEL - COMPREHENSIVE REPORT\n",
          paste(rep("=", 60), collapse = ""), "\n\n",

          "REPORT INFORMATION:\n",
          "Generated on:", format(Sys.time(), "%Y-%m-%d at %H:%M:%S"), "\n",
          # This block is provenance: it travels with a downloaded report into
          # someone else's analysis, so it names the tool, where it lives and
          # which engine version produced the numbers.
          "Analysis Software: Spectral DEMATEL\n",
          "Available at: https://pdelias.github.io/DEMATEL.Sensitivity/\n",
          "Engine: spectralDEMATEL ",
